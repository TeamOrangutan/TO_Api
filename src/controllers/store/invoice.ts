...
export const getInvoiceById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const facturasConPaths = facturas.map((factura) => ({
      ...factura,
      facItems: factura.facItems.map((item) => {
        const imagenes = item.orden_item_fk.producto.imagenes || [];
        const path = imagenes[0]?.url || "";
        const hoverPath = imagenes[1]?.url || "";

        const { imagenes: _, ...productoSinImagenes } =
          item.orden_item_fk.producto;
